...
pub static ANY_TEST_FAILED: AtomicBool = AtomicBool::new(false);
...
/// Returns `true` if any test in the current run has failed.
pub fn is_any_test_failed() -> bool {
    ANY_TEST_FAILED.load(Ordering::SeqCst)
}
...
macro_rules! run_test {
	($test_block:expr) => {{
		...
		//use crate::test_utils::ANY_TEST_FAILED;
		...
		if result.is_err() {
            ANY_TEST_FAILED.store(true, Ordering::SeqCst);
